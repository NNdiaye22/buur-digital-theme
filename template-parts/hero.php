...
/**
 * BUUR Digital — Hero
 * Fond : video uploadee ou video par defaut OU photo via Customizer.
 */

$bg_type    = get_theme_mod( 'buur_hero_bg_type', 'video' );
$bg_img_id  = absint( get_theme_mod( 'buur_hero_bg_image', 0 ) );
$bg_vid_id  = absint( get_theme_mod( 'buur_hero_bg_video', 0 ) );
$badge      = get_theme_mod( 'buur_hero_badge',   'Dakar, Senegal — Agence Web Premium' );
...
$btn_sn     = get_theme_mod( 'buur_hero_btn_sn',   'WhatsApp Senegal' );
...
<section class="hero-section" id="hero" aria-label="Hero BUUR Digital">
  ...
  <div class="hero-video-bg" aria-hidden="true">
    <?php if ( $bg_type === 'image' ) :
        $img_url = $bg_img_id ? wp_get_attachment_image_url( $bg_img_id, 'buur-hero' ) : '';
        if ( $img_url ) : ?>
          <img class="hero-video hero-bg-img" src="<?php echo esc_url( $img_url ); ?>" alt="" loading="eager" decoding="async">
        <?php endif;
    else :
        $vid_url = $bg_vid_id ? wp_get_attachment_url( $bg_vid_id ) : BUUR_URI . '/assets/videos/hero-loop.mp4';
    ?>
      <video id="hero-video" class="hero-video" autoplay muted loop playsinline preload="auto" aria-hidden="true">
